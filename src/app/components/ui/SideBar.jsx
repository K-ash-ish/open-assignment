...
function SideBar() {
  ...
  return (
    <nav
      ...
      } w-4/5 border-2 border-orange-500 rounded-xl capitalize bg-black text-white flex flex-col justify-around items-center px-8 transition-all ease-in`}
    >
      <div className="flex flex-col md:gap-10 gap-4 h-3/5 w-full  ">
        <div className="flex justify-between mt-2 md:m-0">
          ...
          <Image
            ...
            alt="hamburger"
            src={"/bars-solid.svg"}
            className="cursor-pointer md:hidden visible"
            onClick={() => {
              setSidebar(!sidebar);
            }}
          />
        ...
        <div
          ...
          } md:visible md:flex flex-col gap-4 font-extralight ease-in-out`}
        >
          {navItems.map((item) => (
            <NavItem
